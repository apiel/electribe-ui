...
function onMIDIMessage({ data  }) {
    ...
    switch(headers){
        case '240,66,48,0,1,34,64':
        case '240,66,48,0,1,35,64':
...
function parsePattern(rawData) {
    ...
    const name = data.slice(26, 43).filter((c, k)=>c && k != 13
...
document.getElementById('note').innerHTML = Array(255).fill().map((_, key)=>`<option value="${(key + 24) % 255}">${(key + 24) % 255}</option>`
).join('');
...
document.getElementById('sendHex').onclick = ()=>{
    ...
    const msg = data.replaceAll('0x', '').replaceAll(',', ' ').replace(/\s\s+/g, ' ') // replace multiple space to one
    .split(' ').map((v)=>parseInt(`0x${v}`)
    );
    console.log(`Send to ${output.name}:`, msg);
    midiOutput.send(msg);
};
